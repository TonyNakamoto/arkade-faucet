/**
 * Garden footer: Lightning → Ark via Boltz (same identity as topup_lightning.js / faucet).
 */
import {
  Wallet,
  MnemonicIdentity,
...
async function makeLightning() {
  ...
  const result = await swaps.createLightningInvoice({
    amount,
    /* Boltz rejects some unicode in invoice memo; keep ASCII-only. */
    description: "arkade garden tip",
  });
  console.log(
    JSON.stringify({
...
async function main() {
  ...
  if (!cmd || !["limits", "create", "claim"].includes(cmd)) {
    throw new Error("Usage: node donation_lightning.js <limits|create|claim> [amount|pendingSwapJson]");
  }
  if (cmd === "limits") {
